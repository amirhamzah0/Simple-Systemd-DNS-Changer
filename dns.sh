...
echo "  2) Mullvad (Block ads, trackers, malware)"
echo "  3) ControlD (Block ads, trackers, malware)"
echo "  4) Cloudflare (Normal)"
echo "----------------------------------"
...
case $choice in
    ...
    2)
        if [ -f "$DIR/$MULLVAD.bak" ]; then
            ...
            PROFILE="Mullvad DNS (Block ads, trackers, malware)"
        else
    ...
    3)
        if [ -f "$DIR/$CONTROLD.bak" ]; then
            ...
            PROFILE="ControlD DNS (Block ads, trackers, malware)"
        else
